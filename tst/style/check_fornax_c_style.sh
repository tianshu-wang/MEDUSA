...
# SCRIPT: check_fornax_c_style.sh; adapted from check_athena_cpp_style.sh
# AUTHOR: (fornax version) Matt Coleman - msbc@astro.princeton.edu
#         (athena version) Kyle Gerard Felker - kfelker@princeton.edu
# DATE:   (fornax version) 11/22/2021
#         (athena version) 04/18/2018
# PURPOSE:  Wrapper script to ./cpplint.py application to check Fornax src/ code
...
echo "Starting Google C/C++ Style cpplint.py test"
set -e
# Use "python[23] -u" to prevent buffering of sys.stdout,stderr.write() calls in cpplint.py and mix-up in Jenkins logs,
find $srcdir -type f \( -name "*.c" -o -name "*.h" \) -print | xargs python -u ./cpplint.py --counting=detailed
set +e
echo "End of Google C/C++ Style cpplint.py test"

# Begin custom Fornax style rules and checks:
...
done < <(find $srcdir -type f \( -name "*.c" -o -name "*.h" \) -print)
...
find $srcdir -type f \( -name "*.c" -o -name "*.h*" \) -exec grep -n -E " +$" {} +
if [ $? -ne 1 ]; then echo "ERROR: Found C file(s) with trailing whitespace"; exit 1; fi
...
echo "Checking for correct file permissions in $srcdir"
...
git ls-tree -r --full-tree HEAD $(basename ${srcdir}) | awk '{print substr($1,4,5), $4}' | grep -v "644"
