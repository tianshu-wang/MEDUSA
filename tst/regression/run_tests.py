...
# Fornax modules
import scripts.utils.fornax as fornax  # noqa

logger = logging.getLogger('fornax')
...
def main(**kwargs):
    ...
    # Get args to pass to scripts.utils.fornax as list of strings
    fornax_config_args = kwargs.pop('config')
    fornax_run_args = kwargs.pop('run')
        ...
        for name in test_names:
            ...
            try:
                ...
                if coverage_cmd is not None:
                    module.fornax.global_test_name = lcov_test_name
                    module.fornax.global_coverage_cmd = coverage_cmd

                # insert arguments for fornax.run() and fornax.configure()
                # by changing global values through module
                module.fornax.global_config_args = fornax_config_args
                module.fornax.global_run_args = fornax_run_args
                module.fornax.global_silent = silent_opt
                ...
                try:
                    ...
                    # coverage analysis after the final fornax.run() call, producing
                    ...
                    if run_ret is None:
                        fornax.analyze_code_coverage(lcov_test_name, '')
                try:
                ...
                logging.getLogger('fornax.tests.' + name).debug(msg)
...
class MakeFilter(logging.Filter):
    ...
    def filter(self, record):
        return 'fornax.make' != record.name[:len('fornax.make')]
...
def log_init(args):
    ...
    if kwargs.pop('diagnostics_file'):
        ...
        record = logger.makeRecord('fornax.tests', logging.INFO, None, None,
                                   'Starting new tests', None, None)
        rtd_handler.emit(record)
        for log in ['fornax.run', 'fornax.tests']:
            logging.getLogger(log).addHandler(rtd_handler)

    logger.debug('Starting Fornax regression tests')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", "-c",
                        default=[],
                        action='append',
                        help='arguments to pass to fornax.configure')

    parser.add_argument("--run", "-r",
                        default=[],
                        action='append',
                        help='arguments to pass to fornax.run')
