...
const double sigma0 = M_PI/8.0;  // "radius" of cosine bell
const double thetac = 0.0; //-0.1*M_PI;  //0.5*M_PI;  // center of cosine bell
const double phic = 0.5*M_PI;  // center of cosine bell
...
void init_problem()
{
  ...
  p0 = rho_floor*SQR(cs)/gam;
  u0 = p0/(gam-1.0);
  e_floor = p0/(gam-1.0);

  ZLOOP {
    NDP_ELEM(sim.p,ii,jj,kk,RHO) = rho0*Q(ii,jj,kk) + rho_floor;
...
double Q(int i, int j, int k)
{
  ...
  s = DJS(i);
  while (s >>= 1) lev++;
  theta = th_of_x(beta1s[lev][j]/Gamma1s[lev][j]);
  phi = startx[2] + (k+0.5)*DKS(i,j)*dx[2];
  
  sigma = acos(cos(thetac)*cos(theta) + sin(thetac)*sin(theta)*cos(phi-phic));

  return (sigma < sigma0) ? rho0*0.5*(1.0 + cos(M_PI*sigma/sigma0)) : 0.0;
}
